  ...
  ;(() => {
    ...
    Ka({ UA: () => Jn }),
      Ya.deps.push(
        M.all([
          ...
          P ? [] : ba.getAll(),
        ]).then(([{ os: e, arch: a }, { name: t, version: n } = {}, { [Kn]: o, mobile: i } = {}, [s]]) => {
          !n &&
            null != o &&
            o[0] &&
            ([t, n] = o
            ...
            (Jn[Kn] = o),
            (Jn.mobile = i),
            Xn ? (Xn = parseFloat(n)) : s ? Zn(s) : E(ne, ba.onCreated, Zn)
        })
      )
    const eo = ((e, a, t, n, o) => {
      const i = {},
    ...
    async function Zi(e) {
      ...
      const { config: m, props: h } = c,
        g = Uo({ meta: t, props: { id: u } })
      if (d) {
        if (e.isNew || (u && gi.some(({ props: e }) => g === e.uri && u !== e.id))) throw He("msgNamespaceConflict")
        delete c[se]
      }
      ;(h.lastModified = l), (h.uuid = h.uuid || Ji())
      ...
      const p = +e.position
      p ? ((h.position = p), (di = Math.max(di, p))) : d || (di++, (h.position = di)),
        (m.enabled = vi(m.enabled)),
        (m.removed = 0),
        (m.shouldUpdate = vi(m.shouldUpdate)),
        (c.meta = t),
        (h.uri = Uo(c)),
        !Ca(c) && ma(e.from) && (c.custom.homepageURL = e.from),
        da(e.url) && (c.custom.lastInstallURL = e.url),
        (c.custom.tags =
          null == (a = c.custom.tags) ? void 0 : a.split(/\s+/).map(pa).filter(q).join(" ").toLowerCase()),
        r || bt.mod.remove(Aa(c, { all: !0 }) || []),
        Qi(c, e.url)
      const f = es(c, e),
        k = !d || s !== (await bt[rt].getOne(u))
      return (
        k && e.bumpDate && (h.lastUpdated = l),
        e.cache && (await f),
        await bt.base.set({ [ht + u]: c, ...(k && { [lt + u]: s }) }),
        I.assign(o, c, r),
        (i.where = { id: u }),
        (i[rt] = e[rt]),
        ja("UpdateScript", i),
    ...
    const Kc = `${W}confirm/index.html#`,
      Jc =
        /^https:\/\/((greas|sleaz)yfork\.org\/scripts\/[^/]*\/code|update\.(greas|sleaz)yfork\.org\/scripts|openuserjs\.org\/install\/[^/]*|github\.com\/[^/]*\/[^/]*\/(raw\/[^/]*|releases\/download\/[^/]*)|raw\.githubusercontent\.com(\/[^/]*){3}|gist\.github\.com\/.*?)\/[^/]*?\.user\.js([?#]|$)/i,
